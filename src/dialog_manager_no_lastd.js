...
class DialogManager {
  ...
  async execute(userId, intents, entities) {
    ...
    for (const intent of intents) {
      if (this.acceptIntent(intent.value)) {
        const dialogIndex = dialogs.findIndex(d => d.label === intent.label);
        // if no dialogs with that label is found
        if (dialogIndex === -1) {
  ...
  async executeDialogs(userId, entities) {
    ...
    if (dialogs.length > 0) {
      const currentDialog = Object.assign({}, dialogs[dialogs.length - 1]);
      const dialogPath = `${this.config.path}/src/controllers/dialogs/${currentDialog.label}`;
      console.log('DM.executeDialogs: dialogPath', dialogPath);
      const DialogConstructor = require(dialogPath);
      const dialog = new DialogConstructor(currentDialog.parameters);
      const isDone = await dialog.execute(this, userId, entities);
      if (isDone) {
        // if dialog is done then pop it from dialogs
        await this.brain.userPop(userId, 'dialogs');
        // then continue executing the stack
